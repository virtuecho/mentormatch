...
import { reviewMentorRequest, submitMentorRequest, toggleRole, listMentorRequests } from './index';
...
class ProfileTestDatabase implements DatabaseClient {
	...
	private users: UserRow[] = [
		{ id: 1, email: 'mentor@example.com', role: 'mentee', is_mentor_approved: 0 },
	...
	async get<T>(sql: string, params: QueryParams = []): Promise<T | null> {
		...
		if (sql.includes('SELECT id, status') && sql.includes('FROM mentor_requests')) {
			const userId = Number(params[0]);
			const request = [...this.mentorRequests]
				.filter((item) => item.user_id === userId)
				.sort((left, right) => right.submitted_at.localeCompare(left.submitted_at))[0];

			return (request ? { id: request.id, status: request.status } : null) as T | null;
		}
	...
	async all<T>(sql: string): Promise<T[]> {
		if (sql.includes('FROM mentor_requests r')) {
			return this.mentorRequests.map((request) => {
				const user = this.users.find((item) => item.id === request.user_id)!;
				const profile = this.profiles.find((item) => item.user_id === request.user_id)!;
	...
	async run(sql: string, params: QueryParams = []): Promise<QueryResult> {
		...
		if (sql.includes('SET document_url = ?, note = ?, status = \'pending\'')) {
			...
			const request = this.mentorRequests.find((item) => item.id === Number(requestId));
			if (!request) {
