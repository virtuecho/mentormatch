...
import { AppError, bookingCreateSchema, bookingRespondSchema } from '@mentormatch/shared';

export async function createBooking(db: DatabaseClient, menteeId: number, input: unknown) {
	...
	if (!slot || slot.is_booked) {
		throw new AppError(400, 'slot_unavailable', 'Slot unavailable or already booked');
	}
...
export async function cancelBooking(db: DatabaseClient, actorId: number, bookingId: number) {
	const booking = await db.get<{ id: number; mentee_id: number; mentor_id: number }>(
	...
	if (booking.mentee_id !== actorId && booking.mentor_id !== actorId) {
		throw new AppError(403, 'booking_access_denied', 'Booking does not belong to the current user');
	}

	await db.run("UPDATE bookings SET status = 'cancelled', updated_at = ? WHERE id = ?", [
...
export async function respondToBooking(
	...
	const booking = await db.get<{ id: number; mentor_id: number; availability_slot_id: number; status: string }>(
	...
	if (booking.mentor_id !== mentorId) {
		throw new AppError(403, 'booking_access_denied', 'Unauthorized: You are not the assigned mentor');
	}

	if (booking.status !== 'pending') {
		throw new AppError(400, 'booking_not_pending', 'Booking is no longer pending');
	}
	...
	if (payload.response === 'accepted') {
		await db.run('UPDATE availability_slots SET is_booked = 1, updated_at = ? WHERE id = ?', [
			now,
...
export async function listBookings(db: DatabaseClient, userId: number, role: 'mentee' | 'mentor') {
	...
	return rows.map((row) => ({
		...
		counterpart: {
			...
			profileImageUrl: row.counterpart_image,
			email: row.counterpart_email
		}
	}));
}

export async function getBookingHistory(db: DatabaseClient, userId: number, role: 'mentee' | 'mentor') {
	...
	return bookings.filter((booking) => {
		...
		const end = new Date(booking.slot.startTime).getTime() + booking.slot.durationMins * 60 * 1000;
		return end < now;
	});
}
